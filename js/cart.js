        ...
        document.getElementsByTagName("input")[i].addEventListener("change", function(){
            ...
            for(let b= 0; b < cartInfo.articles.length; b++){
                ...
                //Agarra el elemento subtotal del HTML y le pone el valor dinámico del precio
                   ...
                   document.getElementById("totalGlobal").innerHTML = "UYU" + " " + subtotalFinal;
                }
        });
...
//función para mostrar el subtotal cuando la página se carga por primera vez. La repito para poder mostrar el total cuando cargue la página por primera vez nada más.
function subtotalGlobal (){ 
    let subtotalFinal = 0;
    for(let b= 0; b < cartInfo.articles.length; b++){
    individualPriceProduct = document.querySelectorAll(".productSubTotal")[b].innerHTML;
...
function shippingPrice () {  
    //recorro cada opción de envío y le asigno un evento change a cada uno de ellos. Coloco 3 en lugar de lenght porque ya sé la cantidad de opciones de tipo input. 
    for(let i = 0; i < 3; i++){
        document.getElementsByName("carrito")[i].addEventListener("change", function(){
            ...
            if (document.getElementsByName("carrito")[i] == document.getElementsByName("carrito")[0]) {
                document.getElementById("shippingCost").innerHTML = 0.15;
                
            } 
            
            else if (document.getElementsByName("carrito")[i] == document.getElementsByName("carrito")[1]) {
                document.getElementById("shippingCost").innerHTML = 0.07;
            } 
            
            else {
                document.getElementById("shippingCost").innerHTML = 0.05;
            }
        });
...
document.addEventListener("DOMContentLoaded", function(e){
    ...
    getJSONData(CART_INFO_URL).then(function(resultObj){
        if (resultObj.status === "ok")
        {
            ...
            subtotalGlobal (cartInfo);
            shippingPrice (cartInfo);
